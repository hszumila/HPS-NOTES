...
\item
Open the the FEB Power GUI (svtFebMain.adl) shown in Fig.~\ref{fig:svt_FrontEnd}. See~\ref{sec:monitoring_power} to restart GUI if not running.
\item
At the top of the GUI: press the {\sc On} switch marked {\sc DIGI}. Wait until the corresponding channel status indicator turns {\sc GREEN} for all ten FEBs. 
\item
...
\begin{enumerate}
\item
Open the to SVT Flange Power Control GUI, see Fig.~\ref{fig:svt_Flange}  (see~\ref{sec:monitoring_power} to restart if not running).
\item
Press the {\sc ON} switch for the {\sc ALL SVT FLANGES} section at the bottom of the GUI. Wait until the status indicator turns {\color{green} {\sc GREEN}}  for every one of the four flange boards, and check for alarms after currents stabilized. 
\end{enumerate}
...
\item 
Find the SVT Hybrid GUI (svtHybrid.adl) in Fig.~\ref{fig:svt_Status_Main}.
\item
Press the {\sc On} switch marked {\sc All hybrids ON/OFF} to turn on power to the entire SVT.
\item
Wait until all status indicators turn {\sc GREEN}, currents are seen updating and no alarms go off.\newline
...
\begin{enumerate}
\item
Make sure that beam conditions and SVT position has granted permission to turn on HV. {\bf ask the shift leader if you are not sure}.
\item
Find the SVT High Voltage Bias Control GUI (svtBiasMain.adl) shown in Fig.~\ref{fig:svt_BIAS} (see~\ref{sec:monitoring_power} to restart if not running).
\item
...
\item
Wait until all status indicators turn {\color{green} {\sc GREEN}} and no alarms go off.
\end{enumerate}
...
\begin{enumerate}
\item Log into the {\sc clonioc1} computer in the counting house.
...
\begin{itemize}
\item {\bf DAQ IOC Status} in Fig.~\ref{fig:svtDaqIOCStatus}.
\begin{itemize}
...
\item {\bf DPM Status} in Fig.~\ref{fig:svtDpmStatus}.
\begin{itemize}
...
\item {\bf DPM  Link Status} in Fig.~\ref{fig:svtDpmLinkStatus}.
\begin{itemize}
...
\item {\bf Temperature} in Fig.~\ref{fig:svtTemp}.
\begin{itemize}
...
\item {\bf Hybrid GUI} in Fig.~\ref{fig:svtHybrid}.
\begin{itemize}
...
\item {\bf Hybrid Sync} in Fig.~\ref{fig:svtHybridSync}.
\begin{itemize}
...
\item {\bf FEB Power GUI} in Fig.~\ref{fig:svtFebMain}.
\begin{itemize}
...
\item {\bf Flange power GUI} in Fig.~\ref{fig:svtFlange}.
\begin{itemize}
...
\item {\bf High voltage bias GUI} in Fig.~\ref{fig:svtBias}.
\begin{itemize}
