...
\begin{figure*}[t]
	\begin{center}
		\includegraphics[width=0.5\textwidth]{svt_softinterlocks.png}
...
\begin{figure*}[t]
	\begin{center}
		\includegraphics[width=0.5\textwidth]{figures/svt_svtChiller.png}
		\caption{SVT chiller GUI, accessed through the \textbf{Devices} menu in \textbf{hps\_epics}.}
		\label{fig:ctrl_cooling_svtchiller}
	\end{center}
\end{figure*}

\begin{figure*}[t]
	\begin{center}
		\includegraphics[width=0.5\textwidth]{figures/svt_febChiller.png}
		\caption{FEB chiller GUI, accessed through the \textbf{Devices} menu in \textbf{hps\_epics}.}
		\label{fig:ctrl_cooling_febchiller}
	\end{center}
\end{figure*}
...
The fluid is supplied in 10-pound jugs. It is nontoxic and evaporates rapidly.
...
\begin{enumerate}
\item Disable the PLC and software interlocks for supply RTD and return RTD.
...
\subsection{Response to unexpected HV loss}
\label{sec:proc_cooling_HVloss}
If the beam software interlock is enabled, and the beam is off, EPICS will turn off all HV channels.

\begin{enumerate}
\item Verify that all HV channels are off (check the SVT HV bias GUI), but LV channels are on (check the FEB power and flange power GUIs). If LV channels are also off, stop following this procedure. Go to procedure \ref{sec:proc_cooling_mpodtrip}.
\item Check the BPMs (see the scaler GUI or strip charts). If there has not been a beam trip recently, call the SVT expert.
\item If there has been a beam trip, follow the procedure for recovering from a beam trip, \ref{sec:proc_general_beamtrip}.
\end{enumerate}

\subsection{Response to unexpected MPOD trip}
\label{sec:proc_cooling_mpodtrip}
If the MPOD's interlock input is disabled by the PLC, all HV and LV channels will turn off. This should always be accompanied by other trips, which will indicate the cause of the problem.

\begin{enumerate}
\item If all HV channels are off (check the SVT HV bias GUI), but LV channels are on (check the FEB power and flange power GUIs), stop following this procedure. Go to the procedure for recovering from HV loss, \ref{sec:proc_cooling_HVloss}.
\item Look at the MPOD status web page (\texttt{http://hpsmpod}). At the top, next to \textbf{Mainframe Status}, it should read \textbf{\color{red} FAST\_OFF}. If it does not, the MPOD is not tripped (or it tripped momentarily). Call the SVT expert.
\item Check the status of both chillers (second line of each chiller's EPICS screen). If either chiller is stopped, stop following this procedure. Go to the procedure for recovering from a chiller trip, \ref{sec:proc_cooling_chillertrip}.
\item Call the SVT expert if none of the previous steps resolved the problem.
\end{enumerate}
