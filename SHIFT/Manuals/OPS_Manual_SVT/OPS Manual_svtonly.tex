...
\section{Turning the SVT power and bias ON}
...
\section{Power and high voltage bias monitoring}
...
\item Click the button marked {\sc SVT} to open a drop-down menu.
\item Each of the individual GUIs can be opened by clicking the appropriate button in the menu.
\end{enumerate}
...
\item {\bf Hybrid GUI} in Fig.~\ref{fig:svt_HybridLV}.
\begin{itemize}
\item Monitoring: all measured currents from hybrids. 
\item Control: ON/OFF switch for top and bottom half separately as well as individual hybrids.
\item Links: {\sc BIAS}, {\sc FE Low Voltages} and {\sc Flange Boards} buttons open up the GUI for these objects (see below). There is also links to the  individual hybrid power control and monitoring GUI discussed below (EXPERT only).
...
\item Monitoring: all currents and voltages to the flange boards. 
...
\item Control: Set point for voltage, trip and ramp values for each frontend board (EXPERT ONLY).
\end{itemize}
...
\item Control: ON/OFF switch for all modules and each module.  
