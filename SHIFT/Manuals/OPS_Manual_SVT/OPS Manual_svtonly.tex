...
\begin{enumerate}
\item Log into the \textbf{clon01} computer in the counting house.  
\item Open a terminal and issue \textbf{hps\_epics} command to open the main HPS GUI.
\item Click the button marked \textbf{SVT} to open a drop-down menu.
...
\begin{itemize}
%\item \textbf{Main status GUI} in Fig.~\ref{fig:svt_Status_Main}.
%\begin{itemize}
...
%\item Links: \textbf{BIAS}, \textbf{FE Low Voltages} and \textbf{Flange Boards} buttons open up the GUI for these objects (see below). There is also links to the  individual hybrid power control and monitoring GUI discussed below (EXPERT only).
%\end{itemize}

\item \textbf{Hybrid GUI} in Fig.~\ref{fig:svt_HybridLV}.
\begin{itemize}
\item Monitoring: all measured currents from hybrids. 
\item Control: ON/OFF switch for top and bottom half separately as well as individual hybrids.
\item Links: \textbf{BIAS}, \textbf{FE Low Voltages} and \textbf{Flange Boards} buttons open up the GUI for these objects (see below). There is also links to the  individual hybrid power control and monitoring GUI discussed below (EXPERT only).
...
\item \textbf{Flange Board GUI} in Fig.~\ref{fig:svt_Flange}.
\begin{itemize}
...
\item \textbf{FEB GUI} in Fig.~\ref{fig:svt_FrontEnd}.
\begin{itemize}
...
\item \textbf{High voltage bias GUI} in Fig.~\ref{fig:svt_BIAS}.
\begin{itemize}
...
\item
In the \textbf{ALL FEB CONTROL} section at the top, press the \textbf{On} switch marked \textbf{DIGI}. Wait until the corresponding status indicator turns \textbf{GREEN} for every one of the ten FEBs, and no alarms go off.
\item
...
\item
In the \textbf{ALL SVT FLANGES} section at the bottom, press the \textbf{On} switch. Wait until the status indicator turns \textbf{GREEN} for every one of the four flange boards, and no alarms go off.
\item
...
\item
Wait until all status indicators turn \textbf{GREEN} and the \textbf{Measured Voltage} column reads 180V for evey channel, and no alarms go off. If not, call the SVT expert.
\end{enumerate}

\item Turn on SVT Hybrid Power
\begin{enumerate}
\item
Go to the DAQ GUI and do a \textbf{download} with the normal HPS configuration. This configures the SVT FEB which allows the powering of the hybrids. Use the DAQ operations manual if needed. 
\item 
Find the SVT Hybrid GUI in Fig.~\ref{fig:svt_HybridLV} (see~\ref{sec:monitoring_power} to restart if not running).
\item
Press the \textbf{On} switch marked \textbf{All hybrids ON/OFF} to turn on power to the entire SVT.
\item
Wait until all status indicators turn \textbf{GREEN} and no alarms go off.
\item
Check that currents are within range (see Sec.~\ref{sec:svt_power_monitoring_alarm_interlocks} below). If not, follow the procedure outlined in that section.
\end{enumerate}
\end{enumerate}
