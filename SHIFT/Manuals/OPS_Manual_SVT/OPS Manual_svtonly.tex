...
\title{HPS SVT Operations Manual \\ v0.2}
\author{Per Hansson, John Jaros, Takashi Maruyama, \\ Tim Nelson\footnote{contact person for this document}, Marco Oriunno, Sho Uemura}
...
The power needed to operate the SVT is supplied by five MPOD MPV8008I low voltage power supply modules inside a 10-slot Wiener EC crate, see Fig.~\ref{fig:ec_crate}. 
\begin{figure}
\begin{center}
...
\end{center}
\end{figure}
The crate is located in a rack in the pie tower with PTFE (teflon) insulated twisted pair copper wires used to bring power to the SVT in the Hall-B alcove. Each of the five modules supplies power to two FEBs. 

In addition to the low voltage power for the SVT, the crate also houses three ISEG EHS F201p\_805F high precision high voltage modules that provide the reverse bias voltage to the silicon sensors of the SVT. The 
three modules each supply bias to SVT L1-3, L4-6 top or L4-6 bottom.  

\subsection{Control}
\label{sec:svt_power_control}
The SVT power supply and bias is fully controlled through GUIs to the EPICS control system. EPICS controls the EC crate directly through its SNMP interface which supply the FEBs with power. The FEBs regulate and distribute power down to the hybrid boards of the SVT sensor modules. The control and monitoring of the hybrid power (and temperature) is handled by an EPICS IOC running on the SVT DAQ crate. It is important to note that having cooling running is a prerequisite for any power being supplied to the SVT and configuration of the FEB are prerequisite for powering any hybrids.
...
\item
Press the {\sc LV ON/OFF} next to each of the channels.
\item
...
\begin{enumerate}
\item 
Make sure that the cooling for theFEBs is {\sc ON} and {\sc OK}. See Sec.~\ref{sec:svt_cooling} for operation of the cooling system.
\item
...
\item
Press {\sc LV ON/OFF} for each of the three voltages of the ten FEB 
\item
Wait until all status indicators turn {\sc GREEN} and no alarms go off.
\item
...
\begin{enumerate}
\item 
Make sure that the cooling for the SVT hybrids is {\sc ON} and {\sc OK}. See Sec.~\ref{sec:svt_cooling} for operation of the cooling system.
\item
...
\item 
Find the SVT Main Status Control GUI in Fig.~\ref{fig:svt_Status_Main} (see~\ref{sec:monitoring_power} to restart if not running).
\item
Press {\sc SVT TOP ALL ON} to turn on power to the top half of the SVT.
\item
Wait until all status indicators turn {\sc GREEN} and no alarms go off.
\item
Press {\sc SVT BOT ALL ON} to turn on power to the top half of the SVT.
\item
Wait until all status indicators turn {\sc GREEN} and no alarms go off.
\item
Check that currents are within range (see Sec.~\ref{sec:svt_power_monitoring_alarm_interlocks} below). If not, follow the procedure outlined in that section.
...
\item
Check that the set point in the {\sc SVT BIAS ALL V\_SET} is set to 180V. If not call the SVT expert.
\item
Press {\sc SVT BIAS ALL ON/OFF}
\item
Wait until all status indicators turn {\sc GREEN} and no alarms go off.
\end{enumerate}
\end{enumerate}
...
The SVT power and bias as well as alarms are monitored with five principle GUI's. To bring up the GUIs follow these procedures (see slow control manual for more information):
